...
defmodule Iso8583.Transport.TCP.ClientTest do
  ...
  describe "send_and_wait/3" do
    test "returns error when client is not connected" do
      ...
      {:ok, pid} = Client.start_link(
        host: "localhost",
        port: 15599,  # Use a port that's unlikely to be in use
        reconnect_interval: 5000  # Long reconnect interval
      )
      ...
      # Since we can't guarantee connection state, just verify the function is callable
      result = Client.send_and_wait(pid, full_msg, timeout: 100)
      # Should either return an error or timeout (both are acceptable)
      assert result == {:error, :not_connected} or match?({:error, _}, result)
    ...
    test "accepts custom correlation_fields override" do
      ...
      # Should either return an error (not connected) or timeout
      assert result == {:error, :not_connected} or match?({:error, _}, result)
